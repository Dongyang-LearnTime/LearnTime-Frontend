import { XIcon, SparklesIcon, DumbbellIcon, CalendarIcon, ClockIcon, TrendIcon } from '../../../components/ui/Icons';
...
// 백엔드 API 미지원에 따라 화면을 구성하기 위한 로컬 임시값(Mock Data)
const MOCK_REPORT = {
  reportDate: '2026-05-27',
  totalWorkoutTime: 180,
  totalActiveDays: 4,
  totalVolume: 5.2,
  weeklyVolumes: [1.2, 0.8, 1.5, 0, 1.7, 0, 0],
  targetMuscles: ['가슴', '삼두', '하체', '어깨'],
  aiFeedbackSummary: '이번 주는 가슴과 하체 위주의 고중량 볼륨 훈련이 아주 훌륭하게 수행되었습니다. 주 후반부에 휴식일이 잘 배치되어 근회복에 긍정적인 영향을 주었을 것입니다. 다음 주에는 상대적으로 소홀했던 등(광배근) 부위의 볼륨을 조금 더 보강하시는 것을 추천드립니다.'
};

export function ExerciseReportModal({ onClose }: ExerciseReportModalProps) {
  // 로딩 상태 없이 즉시 임시값 데이터 전달
  const report = MOCK_REPORT;

  return (
    <div className="fixed inset-0 z-120 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white dark:bg-[#1a1a1f] w-full max-w-2xl rounded-[3rem] border border-gray-100 dark:border-[#1a1a1a] shadow-2xl p-8 relative animate-in zoom-in-95 duration-200 overflow-hidden">
        {/* 닫기 버튼 */}
        <button
          onClick={onClose}
        ...
        <div className="space-y-6 max-h-137.5 overflow-y-auto pr-2 custom-scrollbar animate-in fade-in duration-500">
          {/* 리포트 정보 및 날짜 */}
          <div className="flex justify-between items-center bg-gray-50 dark:bg-[#202026] rounded-2xl px-6 py-4 border border-gray-100/50 dark:border-[#1a1a1a]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {/* 총 운동 시간 */}
            <div className="p-5 bg-rose-50/30 dark:bg-rose-950/10 border border-rose-100/50 dark:border-rose-900/30 rounded-4xl flex items-center gap-4">
            ...
            {/* 주간 운동 일수 */}
            <div className="p-5 bg-emerald-50/30 dark:bg-emerald-950/10 border border-emerald-100/50 dark:border-emerald-900/30 rounded-4xl flex items-center gap-4">
            ...
            {/* 누적 볼륨 */}
            <div className="p-5 bg-amber-50/30 dark:bg-amber-950/10 border border-amber-100/50 dark:border-amber-900/30 rounded-4xl flex items-center gap-4">
          ...
          {/* 요일별 누적 볼륨 바 차트 */}
          <div className="p-6 bg-gray-50/50 dark:bg-[#18181b]/30 rounded-4xl border border-gray-100/50 dark:border-[#27272a]/50">
            ...
            <div className="flex justify-between items-end h-28 px-2 pt-4">
              {report.weeklyVolumes.map((vol: number, i: number) => {
                const maxVol = Math.max(...report.weeklyVolumes, 1);
                const percent = (vol / maxVol) * 100;
                const DAYS = ['월', '화', '수', '목', '금', '토', '일'];
                return (
                  <div key={i} className="flex flex-col items-center grow group">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 p-4 bg-gray-50/50 dark:bg-[#18181b]/30 rounded-2xl border border-gray-100/50 dark:border-[#27272a]/50">
              {report.targetMuscles.map((muscle: string, i: number) => (
                <span
                  key={i}
          ...
          {/* AI 피드백 총평 */}
          <div className="p-6 bg-gray-50 dark:bg-[#202026] rounded-4xl border border-gray-100 dark:border-[#1a1a1a]">
            <div className="flex items-center gap-2 mb-4">
