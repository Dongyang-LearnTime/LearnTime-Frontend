      ...
      <div className="bg-white dark:bg-[#0a0a0a] w-full max-w-lg rounded-[3rem] border border-gray-100 dark:border-[#1a1a1a] shadow-2xl p-10 animate-in zoom-in-95 duration-200">
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-3 bg-gray-50 dark:bg-[#111] p-1.5 rounded-3xl">
              <button 
                onClick={() => onChangeField('type', 'schedule')}
                className={`grow py-4 rounded-2xl font-black transition-all ${formData.type === 'schedule' ? 'bg-white dark:bg-[#1a1a1a] shadow-lg text-black dark:text-white' : 'text-gray-400'}`}
              >
                일회성 일정
              </button>
              <button 
                onClick={() => onChangeField('type', 'routine')}
                className={`grow py-4 rounded-2xl font-black transition-all ${formData.type === 'routine' ? 'bg-white dark:bg-[#1a1a1a] shadow-lg text-black dark:text-white' : 'text-gray-400'}`}
              >
                반복 루틴
              </button>
            </div>
        ...
        <div className="flex gap-4 mt-12">
          <button 
            onClick={onClose}
            className="grow py-5 bg-gray-50 dark:bg-[#111] text-gray-500 rounded-3xl font-black hover:bg-gray-100 transition-all"
          >
            취소
          </button>
          <button 
            onClick={onSave}
            className="grow py-5 bg-black dark:bg-white text-white dark:text-black rounded-3xl font-black hover:scale-[1.02] active:scale-95 transition-all shadow-xl"
          >
